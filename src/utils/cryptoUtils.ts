...
export const generateSignature = (message: string, secretKey: string): string => {
  ...
  const signature = secretbox(messageUint8, randomBytes(secretbox.nonceLength), secretKeyUint8);
...
export const verifySignature = (
  ...
  try {
    ...
    return secretbox.open(signatureUint8, randomBytes(secretbox.nonceLength), publicKeyUint8) !== null;
  } catch {
    return false;
  }
};